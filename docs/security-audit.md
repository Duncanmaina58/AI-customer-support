...
## Addendum — Sprint 9: AI Action Engine

The Action Engine lets the AI perform real actions (cancel an order, reset a
PIN) against a company's own backend, not just answer questions from the
knowledge base. That's a materially bigger trust boundary than RAG-only
answers — the model can now trigger a real side effect — so this got the same
security-first treatment as the auth rebuild.

### A genuine spec correction, not just an implementation

The Phase 2 spec's own schema names the webhook secret column
`WebhookSecretHash`, matching the pattern used elsewhere in this codebase for
one-way hashes (e.g. `Company.SecretApiKeyHash`). Implemented literally, that
would be a real bug: HMAC-SHA256 webhook signing (the same mechanism this
platform already uses to verify *inbound* Meta/Telegram webhooks — see
Sprint 8's `VerifyMetaSignatureAttribute`) requires the raw shared secret at
signing time, every time. A one-way hash can never be reversed back into that
raw value, so a system that only stored a hash could never actually compute a
valid signature on outgoing calls — it would be able to store a secret but
never use it.

**Fix:** `ActionDefinition.WebhookSecretEncrypted` stores the secret
**reversibly encrypted** (via `ActionEngineSecretProtector`, the same ASP.NET
Core Data Protection mechanism already used for channel credentials in
`ChannelCredentialProtector`), not hashed. Verified end-to-end in
`ActionEngineTests.HandleDetectedActionAsync_executes_webhook_signs_it_and_writes_one_audit_log_on_success`,
which independently recomputes the signature from the *original* raw secret
and confirms it matches what `WebhookActionRunner` actually sent.

### Immutable audit trail — enforced, not just intended

The spec calls for `ActionLogs` to be immutable (a bank/telco compliance
expectation, not just a nice-to-have). "Application code never calls
`.Update()`" is a convention that erodes the first time someone adds an
"edit this log entry" feature under deadline pressure. Instead,
`AppDbContext.SaveChangesAsync` inspects the change tracker on every save and
throws if any `ActionLog` row is found in a `Modified` or `Deleted` state —
regardless of *how* it got there (a stray `.Update()` call, a tracked entity's
property being mutated directly, cascading behavior, anything). Covered by
`ActionEngineTests.ActionLog_cannot_be_updated_after_insert` and
`_cannot_be_deleted`.

### Least privilege

- Mutating endpoints on `ActionDefinitionsController` (create/update/delete/
  regenerate-secret) are Owner/Admin-only — an action definition controls a
  live webhook with a real backend side effect, the same trust tier as
  connecting a channel.
- The audit log endpoint (`GET /api/actions/logs`), which returns *decrypted*
  customer parameters, is also Owner/Admin-only, not open to every
  authenticated Agent.
- Deleting an `ActionDefinition` that already has `ActionLogs` history is
  blocked (409) — the fix is to deactivate it (`IsActive = false`), not
  destroy the audit trail. The FK itself uses `ON DELETE SET NULL` as a
  second layer in case that check is ever bypassed.
- Webhook URLs must be `https://` — enforced server-side in
  `ActionDefinitionsController`, not just suggested in the UI, since the
  payload can include customer data.

### Scope boundaries, stated explicitly rather than silently skipped

- **WhatsApp is not wired into the Action Engine.** Sprint 6's WhatsApp
  webhook already has its own inline pipeline, separate from
  `ChatChannelPipelineService` (used by Messenger/Telegram) by original
  design. Extending it risked regressing a channel that already works,
  for a benefit (action support on one more channel) that can be added
  later with the exact same `ActionEngineService` — nothing about the
  engine itself is web-chat/Messenger/Telegram-specific.
- **Email is not wired in either.** Email's turnaround time (minutes to
  hours between messages) doesn't fit a synchronous "here's your OTP code,
  reply now" exchange well; a store-and-resume design would be a reasonable
  future addition but is a different shape of problem than the other
  channels solve.
- **OTP SMS delivery is a logging placeholder** (`LoggingSmsSender`), not a
  real send — Africa's Talking integration is explicitly Sprint 10's job per
  the Phase 2 build plan (grouped with M-Pesa). The state machine (generate,
  hash, verify, expire, 3-attempt lockout, resume the original action once
  verified) is fully built and tested now, so Sprint 10 only has to write one
  class implementing `ISmsSender` and flip the DI registration — see that
  interface's doc comment and `DependencyInjection.cs`.
- **Sandbox conversations never execute real actions.** Same philosophy as
  sandbox-mode escalation (no real tickets): a company testing their AI
  shouldn't risk firing a real webhook against their own production backend
  by accident. Sandbox messages that would trigger an action get a
  "would trigger in production" message instead.
