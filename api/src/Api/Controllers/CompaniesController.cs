...
public class CompaniesController : ControllerBase
{
    ...
    public async Task<ActionResult<CompanyDetailsDto>> UpdateMe(UpdateCompanyRequest request, CancellationToken ct)
    {
        ...
        if (request.EscalationRulesJson is not null)
        {
            company.EscalationRulesJson = request.EscalationRulesJson;
        }
    ...
    /// <summary>
    /// Onboarding wizard's final step (Step 5, escalation rules): marks onboarding
    /// complete. Requires at least one active channel connection to exist first —
    /// that's Step 4 ("connect your first channel"), enforced here server-side
