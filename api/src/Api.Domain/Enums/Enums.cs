...
// -----------------------------------------------------------------------
// Sprint 9 — AI Action Engine: the AI performs actions (cancel an order,
// reset a PIN) via a company's registered webhook, not just answers
// questions. See Api.Infrastructure.Actions and Api.Domain.Entities.
// ActionDefinition / ActionLog / OtpVerification.
// -----------------------------------------------------------------------

/// <summary>How (if at all) the customer's identity was verified before an action ran — persisted on ActionLog for the audit trail.</summary>
public enum VerificationMethod
{
    /// <summary>Action was read-only (IsReadOnly) or didn't require verification — no identity check performed.</summary>
    None,
    OtpSms,
    OtpWhatsapp,
}
