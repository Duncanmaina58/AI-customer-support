...
public class Company
{
    ...
    /// <summary>
    /// Sprint 7: when the current billing/usage period started. TokensUsedThisMonth
    /// resets to 0 and this rolls forward whenever BillingPeriodResetService finds
    /// a company more than ~30 days past this date — see that class for why a
    /// rolling 30-day window was used instead of calendar-month billing dates.
    /// </summary>
    public DateTime CurrentPeriodStartAt { get; set; } = DateTime.UtcNow;

    /// <summary>
    /// Sprint 7: set the moment the 90%-of-budget warning email is sent, so it
    /// only ever sends once per billing period rather than on every message
    /// after crossing the threshold. Reset to null at the start of each new
    /// period alongside TokensUsedThisMonth — see ConversationService.
    /// IncrementTokenUsageAsync (where the threshold is checked in real time)
    /// and BillingPeriodResetService (where it's cleared on rollover).
    /// </summary>
    public DateTime? TokenBudgetWarningSentAt { get; set; }

    /// <summary>Sprint 7: last phone number used for an M-Pesa payment — prefilled next time, not required.</summary>
    ...
    /// <summary>
    /// Sprint 6: opaque token identifying this company's private sandbox test
    /// chat (platform.com/test/{token}). Separate from PublicApiKey so it can be
    /// shared freely with a team for testing without exposing the production
    /// widget key. Conversations started via this token are flagged
    /// Conversation.IsSandbox and never count against the token budget or
    /// create real tickets — see ChatHub and ChatChannelPipelineService.
    /// </summary>
