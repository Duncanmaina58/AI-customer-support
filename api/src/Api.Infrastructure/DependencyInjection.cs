...
.AddStandardResilienceHandler();
        ...
        services.AddSingleton<IActionEngineSecretProtector, ActionEngineSecretProtector>();

        services.AddHttpClient(ActionWebhookHttpClient.Name, client =>
        {
            client.Timeout = TimeSpan.FromSeconds(35); // per-call CancellationToken enforces the real (per-ActionDefinition) timeout; this is just an outer safety net
            client.DefaultRequestHeaders.UserAgent.ParseAdd("AiSupportPlatformBot/1.0 (+https://aisupportplatform.com/bot)");
        });

        services.AddScoped<IAIActionRunner, WebhookActionRunner>();
        services.AddScoped<IActionRegistry, ActionRegistry>();
        services.AddScoped<IOtpService, OtpService>();
        services.AddScoped<ActionEngineService>();

        // Sprint 10 wires in a real Africa's Talking sender here — swap this one
        // registration and nothing else in the Action Engine needs to change.
