...
import { Plus, Trash2, Pencil, X, BookOpen, AlertCircle, Loader2, FileText } from 'lucide-react'
...
export function ManualEntriesTab() {
  ...
  return (
    <div>
      <div className="animate-reveal mb-4 flex items-center justify-end">
        <button
          type="button"
      ...
      {isLoading && (
        <div className="flex items-center gap-2 text-sm text-muted-400">
          <Loader2 className="h-4 w-4 animate-spin text-teal-400" /> Loading knowledge base…
        </div>
      )}
      ...
      {data?.length === 0 && (
        <div className="animate-reveal flex flex-col items-center justify-center rounded-xl border border-dashed border-ink-700 py-16 text-center">
...
function ChunkCard({
  ...
  return (
    <div className="rounded-xl border border-ink-700 bg-ink-900 p-4 transition-transform hover:-translate-y-0.5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-3">
          <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-teal-500/10 text-teal-400 ring-1 ring-teal-500/20">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-4">
          <p className="text-xs text-muted-400">
            {mode === 'add'
              ? 'Content is embedded immediately with Cohere embed-multilingual-v3.0 and becomes searchable at once.'
