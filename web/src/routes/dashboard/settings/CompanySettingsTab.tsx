...
import type { CompanyDetails, EscalationRules } from '@/lib/types'
...
export function CompanySettingsTab() {
  ...
  const [rulesJustSaved, setRulesJustSaved] = useState(false)
  ...
  useEffect(() => {
    if (company) {
      ...
      setRules(parseEscalationRules(company.escalationRulesJson))
    }
  }, [company])
  ...
  const rulesMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(['company'], data)
      setRulesJustSaved(true)
      setTimeout(() => setRulesJustSaved(false), 2000)
    },
  })
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border border-ink-700 bg-ink-900 p-5">
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-ink-700 bg-ink-950 px-3 py-2 text-sm text-line-200 disabled:opacity-60 focus:border-teal-400 focus:outline-none transition-colors"
          />
        </div>
          ...
          {canEdit && (
            <div className="flex items-center gap-3">
              <button
                ...
                className="rounded-lg bg-teal-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-teal-400 disabled:opacity-60"
              >
                {rulesMutation.isPending ? 'Saving…' : 'Save changes'}
              </button>
              {rulesJustSaved && (
                <span className="flex items-center gap-1 text-sm text-green-500">
                  <Check className="h-4 w-4" /> Saved
