...
import type { ChannelConnection, ConnectWebChatResponse } from '@/lib/types'

export function Step4ConnectChannel({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <EmailChannelCard channels={channels} canEdit={true} />
      <MessengerChannelCard channels={channels} canEdit={true} />
      <TelegramChannelCard channels={channels} canEdit={true} />
      ...
      {isExpanded && !isConnected && (
        <form onSubmit={handleConnect} className="mt-3 space-y-2">
          <input
            ...
            className="w-full rounded-md border border-ink-700 bg-ink-950 px-2.5 py-1.5 text-sm text-line-200 placeholder:text-muted-400 focus:border-teal-400 focus:outline-none transition-colors"
          />
          <input
            required
            value={phoneNumberId}
      ...
      {isConnected && (
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-md border border-ink-700 bg-ink-950 px-2.5 py-1.5 text-sm text-line-200 placeholder:text-muted-400 focus:border-teal-400 focus:outline-none transition-colors"
          />
          <button
            type="button"
