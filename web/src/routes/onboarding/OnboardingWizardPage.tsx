...
import { Step5EscalationRules } from '@/routes/onboarding/steps/Step5EscalationRules'
import { Step6TestAndGoLive } from '@/routes/onboarding/steps/Step6TestAndGoLive'
...
export function OnboardingWizardPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-ink-950 px-4 py-10">
      <AuthBackground />
      <div className="relative z-[2] mx-auto max-w-xl animate-reveal">
          ...
          {STEPS.map((step, i) => (
            <li key={step.label} className="flex flex-1 items-center last:flex-none">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span className={clsx('hidden text-[11px] sm:block', i === stepIndex ? 'text-line-200' : 'text-muted-400')}>
                  {step.label}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <span className={clsx('mx-2 h-px flex-1 transition-colors duration-300', i < stepIndex ? 'bg-teal-500' : 'bg-ink-700')} />
              )}
            </li>
          ))}
        </ol>
        ...
        <div className="rounded-xl border border-ink-700 bg-ink-900 p-6 shadow-2xl">
          ...
          {stepIndex === 3 && <Step4ConnectChannel onNext={goNext} onBack={goBack} />}
          {stepIndex === 4 && <Step5EscalationRules company={company} onNext={goNext} onBack={goBack} />}
          {stepIndex === 5 && <Step6TestAndGoLive onBack={goBack} />}
        </div>
      </div>
    </div>
  )
