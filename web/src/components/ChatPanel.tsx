...
export function ChatPanel({
  ...
  return (
    <div className={`flex flex-col bg-ink-950 ${className}`}>
      <header className="flex items-center gap-2.5 border-b border-ink-700 bg-ink-900/60 px-4 py-3 backdrop-blur-sm">
        <span
        ...
        <div className="leading-tight">
          ...
          <p className="flex items-center gap-1.5 text-[11px] text-muted-400">
            {connectionState === 'connected' && (
              <>
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 trend-dot" /> We typically reply instantly
              </>
            )}
        ...
        {isSandbox && (
          <span className="ml-auto flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-amber-400 ring-1 ring-amber-500/20">
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-ink-700 bg-ink-900/40 p-3">
        <input
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          placeholder="Type a message…"
          disabled={connectionState !== 'connected'}
          className="flex-1 rounded-full border border-ink-700 bg-ink-900 px-4 py-2 text-sm text-line-200 placeholder:text-muted-400 transition-colors focus:border-teal-400 focus:outline-none disabled:opacity-60"
        />
        <button
          type="submit"
          disabled={!draft.trim() || connectionState !== 'connected'}
          aria-label="Send message"
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-teal-500 text-white transition-all hover:bg-teal-400 disabled:opacity-50"
        >
